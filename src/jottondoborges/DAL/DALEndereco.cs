...
namespace jottondoborges.App_Code.DAL
{
    public class DALEndereco : DAL
    {
        ...
        [DataObjectMethod(DataObjectMethodType.Select)]
        public static Endereco SelectFromFornecedor(int f)
        {
            Endereco e = new Endereco();
            try
            {
                ...
                {
                    conn.Open();
                    string s = "SELECT * FROM EnderecoFornecedor WHERE Fornecedor_id = @id";
                    SqlCommand c = new SqlCommand(s, conn);
                    c.Parameters.Add("@id", SqlDbType.Int).Value = f;
                    SqlDataReader d;
                    using (d = c.ExecuteReader())
        ...
        [DataObjectMethod(DataObjectMethodType.Insert)]
        public static void InsertFornecedor(Endereco e)
        {
            try
            {
                using (conn = new SqlConnection(connectionString))
                {
                    ...
                    string compl = (e.complemento != "" && e.complemento != null) ? ("Complemento, ") : "";
                    string q = "INSERT INTO EnderecoFornecedor (Estado, Cidade, Bairro, Rua, Numero, " + compl + dono + ") VALUES (@e, @c, @b, @r, @n, " + ((compl != "") ? "@compl, " : "") + "@d)";
                    SqlCommand cmd = new SqlCommand(q, conn);
                    cmd.Parameters.Add("@e", SqlDbType.VarChar).Value = e.estado;
                    cmd.Parameters.Add("@c", SqlDbType.VarChar).Value = e.cidade;
                    cmd.Parameters.Add("@b", SqlDbType.VarChar).Value = e.bairro;
                    ...
                    cmd.Parameters.Add("@n", SqlDbType.Int).Value = e.numero;
                    if (e.complemento != "" && e.complemento != null) cmd.Parameters.Add("@compl", SqlDbType.VarChar).Value = e.complemento;
                    cmd.Parameters.Add("@d", SqlDbType.Int).Value = e.idFornecedor;
                    cmd.ExecuteNonQuery();
                }
            }
            catch (Exception)
        ...
        public static void DeleteFromCliente(int i)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string q = "DELETE FROM Endereco WHERE Cliente_id = @id";
                    SqlCommand cmd = new SqlCommand(q, conn);
        ...
        public static void DeleteFromFuncionario(int i)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string q = "DELETE FROM Endereco WHERE Funcionario_id = @id";
                    SqlCommand cmd = new SqlCommand(q, conn);
                    cmd.Parameters.Add("@id", SqlDbType.Int).Value = i;
                    cmd.ExecuteNonQuery();
                }
        ...
        public static void DeleteFromFornecedor(int i)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string q = "DELETE FROM EnderecoFornecedor WHERE Fornecedor_id = @id";
                    SqlCommand cmd = new SqlCommand(q, conn);
                    cmd.Parameters.Add("@id", SqlDbType.Int).Value = i;
                    cmd.ExecuteNonQuery();
                }
